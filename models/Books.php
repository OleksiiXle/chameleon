...
//-----------------------------------------------------------------------------------------------------------------
    public function rules(){
        return [
            ...
            [['newPreview'], 'file',  'extensions' => 'png, jpg', 'maxSize' => 3000*3000],
        ];
    }
//-----------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------------------
    public function mySave($preview){
        if ($this->isNewRecord) {
            //--- новая запись
            ...
            if (strlen($preview) > 0) {
                //если выбрана картинка - новая запись
                $ext = end(explode(".", basename($preview)));
                $this->preview = 'preview_' . $newID . '.' . $ext;
                $fNameFull = self::getIcon($preview ,$this->pathToPreview, $ext ,$this->preview, 2000);
                $fNameFull = self::getIcon($preview ,$this->pathToPreview, $ext ,'i' . $this->preview, 500);
            } else {
                //если не выбрана картинка - новая запись
                ...
                if (strlen($preview) > 0) {
                    //если выбрана картинка старая запись
                     $previewLabel = date('d-H-i');
                     $ext = end(explode(".", basename($preview)));
                     if (file_exists($this->pathToPreview . $this->preview))
                     ...
                     $this->preview = 'preview_' . $newID . '_' . $previewLabel .'.' . $ext;
                     $fNameFull = self::getIcon($preview ,$this->pathToPreview, $ext ,$this->preview, 2000);
                     $fNameFull = self::getIcon($preview ,$this->pathToPreview, $ext ,'i' . $this->preview, 500);
                 }
            }
